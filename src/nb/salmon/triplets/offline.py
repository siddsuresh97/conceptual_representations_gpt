    ...
    def fit(self, X_train, X_test, embedding=None, get_stats=None, **stats_kwargs):
        ...
        for k in itertools.count():
            ...
            if (self.verbose and k % self.verbose == 0) or abs(
                ...
                running_loss.append(datum['loss_test'])
            ...
            if self.verbose and k % self.verbose == 0:
                # fmt: off
                keys = [
                    "ident","loss_test", "score_test", "train_data", "max_epochs",
            ...
            if len(running_loss)>10:
                print('mean minus current',abs(np.mean(running_loss) - datum['loss_test']),k)
                if abs(np.mean(running_loss) - datum['loss_test'])<sigma:
                    break
                else:
                    running_loss.pop(0)
